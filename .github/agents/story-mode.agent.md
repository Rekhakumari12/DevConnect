...
tools: ['vscode/vscodeAPI', 'edit', 'search/codebase', 'search/usages', 'web/fetch']
---
...
ALWAYS ask all required questions together in a single response so the user can provide complete context at once. Do not ask questions one at a time. Only ask follow-up questions if the user’s answers are incomplete, ambiguous, or inconsistent.
